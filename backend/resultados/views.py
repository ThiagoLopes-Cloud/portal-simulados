...
def calcular_dashboard(aluno):
    ...
    resultados = (
        Resultado.objects.filter(aluno=aluno)
    ...
    todas_respostas = Resposta.objects.filter(
        questao__tema__isnull=False
    ...
    respostas_aluno = Resposta.objects.filter(
        aluno=aluno, questao__tema__isnull=False
    ).select_related("questao__tema__materia")
    ...
    for resposta in respostas_aluno:
        ...
        if mid not in dados:
            dados[mid] = {"nome": materia.nome, "codigo": materia.codigo, "temas": {}}
    ...
    for mid, materia_data in dados.items():
        ...
        for tid, tema_data in materia_data["temas"].items():
            ...
            media_tema = (
                round(bt["acertos"] / bt["total"] * 100, 1) if bt["total"] > 0 else 0
            )
        ...
        percentual_materia = (
            round(acertos_materia / total_materia * 100, 1) if total_materia > 0 else 0
        )

        bm = benchmark_materia.get(mid, {"total": 0, "acertos": 0})
        media_materia = (
            round(bm["acertos"] / bm["total"] * 100, 1) if bm["total"] > 0 else 0
        )
...
class ResultadoDetalheView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response(
                {"error": "Resultado não encontrado."}, status=status.HTTP_404_NOT_FOUND
            )
...
class RankingView(APIView):
    ...
    def get(self, request):
        ...
        melhores_ids = (
            Resultado.objects.values("aluno", "simulado")
        ...
        resultados = (
            Resultado.objects.filter(id__in=melhores_ids)
...
class GabaritoView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            resultado = Resultado.objects.select_related("simulado", "aluno").get(
                pk=pk, aluno=request.user
            )
        except Resultado.DoesNotExist:
            return Response(
                {"error": "Resultado não encontrado."}, status=status.HTTP_404_NOT_FOUND
        ...
        simulado_questoes = (
            SimuladoQuestao.objects.select_related(
                "questao", "questao__tema", "questao__tema__materia"
            )
        ...
        respostas_dict = {
            ...
            for r in Resposta.objects.filter(
                aluno=request.user, simulado=simulado, tentativa=resultado.tentativa
            )
        }
        ...
        for sq in simulado_questoes:
            ...
            materia_codigo = (
                questao.tema.materia.codigo
                if questao.tema and questao.tema.materia
                else None
            )
            materia_nome = (
                questao.tema.materia.nome
                if questao.tema and questao.tema.materia
                else None
            )
...
class EvolucaoSimuladoView(APIView):
    ...
    def get(self, request, simulado_id):
        resultados = Resultado.objects.filter(
            aluno=request.user, simulado_id=simulado_id
        ).order_by("tentativa")
