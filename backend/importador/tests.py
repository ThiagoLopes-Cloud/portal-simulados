...
class ImportadorParsingTests(TestCase):
    ...
    def test_parse_gabarito_extracts_ingles_e_espanhol(self):
        text = "QUESTAO GABARITO INGLES ESPANHOL\n" "1 D B\n" "2 D A\n" "5 A C\n" "6 E"
        self.assertEqual(
            parse_gabarito(text),
            {
    ...
    def test_clean_extracted_field_removes_pdf_header_noise(self):
        ...
        self.assertEqual(
            cleaned, "desigualdade de genero acentuada pela baixa escolarizacao."
        )
    ...
    def test_question_has_visual_hint_detects_chart_and_figure_terms(self):
        ...
        self.assertTrue(
            question_has_visual_hint("A figura a seguir apresenta um mapa.")
        )
        self.assertTrue(
            question_has_visual_hint(
        ...
        self.assertFalse(
            question_has_visual_hint("Texto puramente conceitual sem apoio visual.")
        )
    ...
    def test_select_owner_block_for_page_without_visual_hint_uses_last_starting_block(
        self,
    ):
        blocks = [
            {
...
class ImportacaoDeleteBehaviorTests(TestCase):
    def test_can_delete_importacao_even_when_simulado_has_results(self):
        ...
        prova = ProvaOriginal.objects.create(
            importacao=importacao, descricao="ENEM 2025"
        )
        ...
        simulado.resultados.create(
            aluno=user, tentativa=1, acertos=1, total_questoes=1, score=100
        )
...
class DedupeQuestaoEntreProvasTests(TestCase):
    def test_publicacao_reaproveita_questao_existente_em_outra_prova(self):
        user = User.objects.create_user(
            username="admin2", password="senha123", role="admin"
        )
        ...
        prova_2009 = ProvaOriginal.objects.create(
            importacao=importacao_2009, descricao="ENEM 2009 - Azul"
        )
        ...
        prova_2025 = ProvaOriginal.objects.create(
            importacao=importacao_2025, descricao="ENEM 2025 - Branco"
        )
        Simulado.objects.create(
        ...
        self.assertEqual(
            provas,
            [
                (2009, ImportacaoProva.COR_AZUL, 1),
                (2025, ImportacaoProva.COR_BRANCO, 77),
            ],
        )

    def test_publicacao_nao_reaproveita_questao_quando_idioma_e_diferente(self):
        user = User.objects.create_user(
            username="admin3", password="senha123", role="admin"
        )
        ...
        prova = ProvaOriginal.objects.create(
            importacao=importacao, descricao="ENEM 2025 - Azul"
        )
        Simulado.objects.create(
            titulo="ENEM 2025 - Dia 1 - Azul",
            descricao="Original",
        ...
        self.assertEqual(
            Questao.objects.filter(enunciado="Texto equivalente.").count(), 2
        )
...
class TemaEDificuldadeImportadasTests(TestCase):
    def test_questao_importada_nasce_com_dificuldade_media_por_padrao(self):
        user = User.objects.create_user(
            username="admin4", password="senha123", role="admin"
        )
        importacao = ImportacaoProva.objects.create(
            ano=2025,
            dia=1,
    ...
    def test_publicacao_propaga_tema_e_dificuldade_para_questao_oficial(self):
        user = User.objects.create_user(
            username="admin5", password="senha123", role="admin"
        )
...
class ImagemEnunciadoImportadaTests(TestCase):
    ...
    def test_build_public_media_url_uses_public_base_url(self):
        user = User.objects.create_user(
            username="admin7", password="senha123", role="admin"
        )
        importacao = ImportacaoProva.objects.create(
            ano=2025,
            dia=1,
    ...
    def test_build_public_media_urls_for_question_includes_alternative_images(self):
        user = User.objects.create_user(
            username="admin7b", password="senha123", role="admin"
        )
        importacao = ImportacaoProva.objects.create(
            ano=2025,
            dia=1,
        ...
        self.assertEqual(
            urls["imagem_opcao_a"],
            f"http://testserver{questao.imagem_opcao_a_arquivo.url}",
        )
        self.assertIsNone(urls["imagem_opcao_b"])
        self.assertEqual(
            urls["imagem_opcao_c"],
            f"http://testserver{questao.imagem_opcao_c_arquivo.url}",
        )

    def test_publicacao_propaga_imagem_enunciado_para_questao_oficial(self):
        user = User.objects.create_user(
            username="admin8", password="senha123", role="admin"
        )
        importacao = ImportacaoProva.objects.create(
            ano=2025,
            dia=1,
    ...
    def test_publicacao_propaga_imagens_das_alternativas_para_questao_oficial(self):
        user = User.objects.create_user(
            username="admin8b", password="senha123", role="admin"
        )
        importacao = ImportacaoProva.objects.create(
            ano=2025,
            dia=1,
    ...
    def test_dedupe_reaproveita_questao_e_completa_tema_ausente(self):
        user = User.objects.create_user(
            username="admin6", password="senha123", role="admin"
        )
        materia = Materia.objects.create(nome="Fisica", codigo="FIS")
