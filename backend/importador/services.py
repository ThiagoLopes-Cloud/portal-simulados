...
PDF_HEADER_RE = re.compile(
    r".*\|\s*\d.{0,2}\s*DIA\s*\|\s*CADERNO\s*\d+\s*\|\s*[A-Z]+.*", re.IGNORECASE
)
...
def clean_extracted_field(text):
    ...
    for line in lines:
        ...
        if cleaned:
            previous = cleaned[-1]
            if BROKEN_WORD_RE.fullmatch(previous) and re.match(
                r"^[a-zà-ÿ]", line, re.IGNORECASE
            ):
            ...
            if previous_match and re.match(r"^[a-zà-ÿ]", line, re.IGNORECASE):
                cleaned[-1] = (
                    f"{previous_match.group(1)}{previous_match.group(2)}{line}"
                )
                continue
...
def fix_fragmented_start(line):
    ...
    def first_token_can_merge(token):
        return (
            token.lower() not in JOIN_START_STOPWORDS
            or token.lower() in JOIN_START_EXCEPTIONS
        )
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            for image_index, image in enumerate(getattr(page, "images", []), start=1):
                ...
                original_name = getattr(
                    image, "name", f"page_{page_number}_image_{image_index}"
                )
                images.append(
                    {
...
def split_question_blocks(text):
    return split_question_blocks_from_pages([{"number": 1, "text": text, "images": []}])
...
def split_question_blocks_from_pages(page_entries):
    ...
    for page_entry in page_entries:
        ...
        for line in page_entry["text"].splitlines():
            ...
            if (
                QUESTION_RANGE_RE.search(line)
                and current_number is None
                and current_language
                and "01 a 05" not in line
            ):
                current_language = None
...
def parse_question_block(block_text):
    ...
    if candidate_sequences:
        start_indexes = min(
            candidate_sequences, key=lambda item: (item["E"] - item["A"], -item["A"])
        )

    if start_indexes:
        enunciado_lines = lines[: start_indexes["A"]]
        for idx, line in enumerate(
            lines[start_indexes["A"] :], start=start_indexes["A"]
        ):
            match = ALT_LINE_RE.match(line)
...
def classify_question(parsed, answer, has_image=False):
    ...
    if (
        question_has_visual_hint(parsed.get("texto_bruto", parsed.get("enunciado", "")))
        and not has_image
    ):
        problems.append("Questao com indicativo visual sem imagem associada.")

    status = (
        QuestaoImportada.CORRECAO_NECESSARIA
        if problems
...
def build_public_media_urls_for_question(questao_importada):
    return {
        "imagem_enunciado": build_public_media_url(
            questao_importada.imagem_enunciado_arquivo
        ),
        "imagem_opcao_a": build_public_media_url(
            questao_importada.imagem_opcao_a_arquivo
        ),
        "imagem_opcao_b": build_public_media_url(
            questao_importada.imagem_opcao_b_arquivo
        ),
        "imagem_opcao_c": build_public_media_url(
            questao_importada.imagem_opcao_c_arquivo
        ),
        "imagem_opcao_d": build_public_media_url(
            questao_importada.imagem_opcao_d_arquivo
        ),
        "imagem_opcao_e": build_public_media_url(
            questao_importada.imagem_opcao_e_arquivo
        ),
    }
...
def select_owner_block_for_page(page_number, all_blocks):
    starting_blocks = [
        block for block in all_blocks if block.get("pagina_inicial") == page_number
    ]
    candidate_blocks = starting_blocks or [
        block for block in all_blocks if page_number in block.get("paginas", [])
    ]
    ...
    visual_blocks = [
        ...
        if question_has_visual_hint(block_hint_text(block))
    ]
...
def select_images_for_block(block, all_blocks, page_entries_by_number):
    page_numbers = block.get("paginas") or (
        [block["pagina_inicial"]] if block.get("pagina_inicial") else []
    )
    page_images = []
...
def save_extracted_image(questao_importada, image_info):
    suffix = (
        ...
        if image_info["extension"].startswith(".")
        else f".{image_info['extension']}"
    )
...
def processar_importacao(importacao):
    ...
    if not gabarito:
        raise ValidationError("Nao foi possivel identificar o gabarito no PDF enviado.")

    max_gabarito = max((numero for numero, _idioma in gabarito.keys()), default=0)
    if max_gabarito:
        question_blocks = [
            item for item in question_blocks if item["numero"] <= max_gabarito
        ]
    ...
    prova_original.total_questoes_esperado = infer_expected_total(
        question_blocks, gabarito
    )
    ...
    for item in question_blocks:
        ...
        image_info, option_image_infos, image_reason = select_images_for_block(
            item, question_blocks, page_entries_by_number
        )
...
def publicar_questao_importada(questao_importada):
    ...
    if questao is None:
        ...
        if questao.dificuldade == "M" and questao_importada.dificuldade in {"F", "D"}:
    ...
    questao_importada.save(
        update_fields=["questao_oficial", "status", "motivo_status", "atualizado_em"]
    )
...
def atualizar_status_importacao(importacao):
    ...
    if hasattr(importacao, "prova_original"):
        ...
        importacao.prova_original.save(
            update_fields=["status_editorial", "atualizado_em"]
        )
