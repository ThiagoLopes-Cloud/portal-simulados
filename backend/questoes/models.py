...
class Questao(models.Model):
    ...
    opcao_e = models.CharField(
        max_length=500, verbose_name="Alternativa E", blank=True, default=""
    )

    # URLs de imagem para cada alternativa — todas opcionais
    imagem_opcao_a = models.URLField(
        blank=True, null=True, verbose_name="Imagem A (URL)"
    )
    imagem_opcao_b = models.URLField(
        blank=True, null=True, verbose_name="Imagem B (URL)"
    )
    imagem_opcao_c = models.URLField(
        blank=True, null=True, verbose_name="Imagem C (URL)"
    )
    imagem_opcao_d = models.URLField(
        blank=True, null=True, verbose_name="Imagem D (URL)"
    )
    imagem_opcao_e = models.URLField(
        blank=True, null=True, verbose_name="Imagem E (URL)"
    )

    # Gabarito — não é exposto na API de prova por segurança
    resposta_correta = models.CharField(
        max_length=1, choices=OPCOES_CHOICES, verbose_name="Resposta Correta"
    )
    ...
    ano_origem = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Ano de Origem"
    )
    ...
    numero_na_prova = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Número na Prova"
    )
