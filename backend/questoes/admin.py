...
def rejeitar_questoes(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, f"{total} questão(ões) rejeitada(s) e removida(s)."
    )
...
class QuestaoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Conteúdo", {"fields": ("enunciado", "imagem_enunciado", "tema")}),
        (
            "Alternativas",
            {
                "fields": (
        ...
        ("Gabarito", {"fields": ("resposta_correta", "explicacao", "dificuldade")}),
        (
            "Metadados e Revisão",
            {
                "fields": (
    ...
    def provas_oficiais_resumidas(self, obj):
        ocorrencias = obj.ocorrencias_prova.select_related(
            "prova_original__importacao"
        ).order_by(
